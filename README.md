...
* [`src/date_range/styles.cljs`](src/date_range/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/date_range/`](src/date_range/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/date_range/core.cljs): contains the SPA entry point, `init`
* [`test/date_range/`](test/date_range/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `date-range` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/date_range/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns date-range.example
  (:require [date-range.config :as config])
